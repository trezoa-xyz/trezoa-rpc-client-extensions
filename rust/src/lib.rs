...
pub fn get_stake_activation(
    ...
    stake: &Pubkey,
) -> ClientResult<StakeActivation> {
    let stake_account = rpc_client.get_account(stake)?;
    let stake_history_account = rpc_client.get_account(&stake_history::id())?;
    let epoch_info = rpc_client.get_epoch_info()?;
    let stake_state = bincode::deserialize::<StakeStateV2>(&stake_account.data).map_err(|_| {
    ...
    let stake_history =
        from_account::<StakeHistory, _>(&stake_history_account).ok_or(Error::from(
    ...
    let StakeActivationStatus {
        ...
        activating,
        deactivating,
    } = delegation.stake_activating_and_deactivating(epoch_info.epoch, &stake_history, Some(0));
